...
def options_menu_gen():
 ...
 3. Default Metrics Path: %s
 4. Default Archive Path: %s
 5. Default BASS Archive Path: %s
...
def options_menu():    
    ...
    while i is not None:
        ...
        if i == 1:
            ...
            change_setting('METRICS','file_path')
        elif i == 4:
...
def stage_5(mcap,metrics):
    ...
    print('Currently only supports Indexed ad check')
...
def create_mcap_path(panel,metrics):
    ...
    if metrics.sheet_name == 'ROP':
        panel2 = panel + 'ROP'
        mcap_path[0] = r'\\mt3fs0.mt3.local\MCAPPull'+'\\'+dtt.datetime.today().strftime('%m%d%Y_QC_Completed_')+panel2+'.txt'
    else:
        mcap_path[0] = r'\\mt3fs0.mt3.local\MCAPPull'+'\\'+dtt.datetime.today().strftime('%m%d%Y_QC_Completed_')+panel+'.txt'
    mcap_path[1] = panel
    return mcap_path
    
def mk_alt_mcap(mcap):
    mcap_path = [0,0]
    mcap_path[0] = r'\\mt3fs0.mt3.local\MCAPPull'+'\\'+dtt.datetime.today().strftime('%m%d%Y_not_QCC_')+mcap.panel+'.txt'
