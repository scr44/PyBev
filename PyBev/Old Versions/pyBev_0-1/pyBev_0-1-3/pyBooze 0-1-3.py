...
Alpha version 0.1.3 (Reorganized & cleaner Stage 1 code)
...
def spotpull():
    ...
    Filters:
    Date range = YTD (1/1/YY - MM/DD/YY) | Status.Descrip = QC Complete
    Attributes:
...
def datemake(datestring):
    """Turns a string into a datetime. Enter date string using format 'MM/DD/YYYY' """
...
def query_YN():
    ...
    if (queryYN == 'Yes' or queryYN == 'y' or queryYN == 'yes'):
        sdpath = input('MCAP data file location (Press Enter for default): ') or r'C:\Users\sroy\Documents\BevAl Metrics\MCAP - Print Data Tool.csv'
    else:
...
def week_init():
    """Creates an initial week object and determines what the checking cutoff to be used in Stage 2 is."""
