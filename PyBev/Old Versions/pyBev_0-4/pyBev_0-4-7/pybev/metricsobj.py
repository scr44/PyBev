...
class MCAPData:
    ...
    def fpivot(self):
        if self.ROP is False:
            ...
            print('Creating pivot table (Count of FIDs)... ',end=''),
            # self.pivot_df = pd.pivot_table(self.df,index='RetMktConcatenated',columns='WeekOf',values='VehicleId', aggfunc=[len])
    ...
    def fpivot_status(self,status):
        ...
        #not QCC data has no WeekOf, need to construct it manually
...
class PanelMetrics:
    ...
    def extract(self):
        ...
        try:
            self.df = pd.read_excel(self.file_path, sheetname=self.sheet_name, index_col=0)
