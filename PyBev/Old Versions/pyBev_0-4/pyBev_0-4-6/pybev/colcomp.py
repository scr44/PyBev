...
def preliminary_update(mcap,metrics):
    ...
    # let's be real, the cutoff is ALWAYS the rundate
    ...
    monthly_items(mcap,metrics,A,B,C,D,cutdt)
    
    ad_hoc_items(mcap,metrics,A,B,C,D,cutdt)
    
    OOB_items(metrics,A,C)
    
    late_added_items(mcap,metrics,A,B)
...
def monthly_items(mcap,metrics,A,B,C,D,cutdt):
    dff = irregular_filter(metrics.df)
    dff = status_filter(dff,A,status='0')
    for x in range(0,len(dff[A])):
        ...
        if (expdt >= A + dtt.timedelta(days=7)
            and cutdt > A + dtt.timedelta(days=10)): # 3-day padding to allow QCing
...
def OOB_items(metrics,A,C):
    ...
    for x in range(0,len(dff[A])):
        ...
        if dff.loc[i,A] == 0:
            ...
            metrics.df.loc[i,'Issue'] = 'Unexpected Drop'
            
    mapping.map_small_to_big(A,dff,metrics.df,agg_func=None)
    return
    
def late_added_items(mcap,metrics,A,B):
