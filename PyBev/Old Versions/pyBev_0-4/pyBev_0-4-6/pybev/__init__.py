...
def menu_string_gen(mcap,metrics):
 ...
 3. Map ad count
 4. Column comparison
 5. Fix & report problems (WIP)
 6. Run program with defaults (WIP)
 7. BASS
 8. Options (WIP)
...
def main_menu(mcap,metrics,report):
    ...
    while True:
        ...
        if type(user_input) is int:
            mcap,metrics,report,volatile_updated = program_stages(
                                user_input,mcap,metrics,report,volatile_updated)
...
def program_stages(stage,mcap,metrics,report,volatile_updated):
    if stage == 1:
        mcap, metrics = stage_1()
    elif stage == 2:
        volatile_updated = stage_2(metrics)
    elif stage == 3:
        volatile_updated = stage_3(mcap, metrics)
    elif stage == 4:
        stage_4(mcap, metrics,volatile_updated)
    elif stage == 5:
    ...
    return mcap,metrics,report,volatile_updated
...
def stage_4(mcap,metrics,volatile_updated):
    ...
    if mcap.ROP is False:
        ...
        metrics.xl_col_sort(key2='Retailer')
...
# def monday_archive(metrics,options):
    ...
    archive_path = r'C:\Users\sroy\Documents\BevAl Metrics'
    
    filename = (archive_path + '\\Archive\\' + 
                    monday_date.strftime('%Y-%m-%d ') + metrics.panel + '.xlsx')
                    
    filename2 = (archive_path + '\\2017 BevAl Metrics.xlsx')
...
'''There is no existing archive for this week. Would you like to archive the Metrics file? y/n: ''') == 'y':
            ...
            print(
'''Data archived to %s
You can change the archive directory in Options.''' % (archive_path + '\\Archive\\'))
