...
def non_qcc_items(mcap,metrics,alt_mcap,status):
    ...
    # B = Status_item_Count
    if np.size(alt_mcap.status_pivot_df) == 0:
        print('No ads with status remaining for %s.' % metrics.week_date.strftime('%m/%d/%Y'))
        return False
    elif mp.valid_index_check(metrics.week_date,alt_mcap.status_pivot_df,'len') is False:
        print('No ads with status remaining for %s.' % metrics.week_date.strftime('%m/%d/%Y'))
        return False
    ...
    # if A == Status and B == 0, A = ?
    dff = cc.status_filter(metrics.df,A,status=status)
    for x in range(0,len(dff.index)):
        i = dff.index.values[x]
        try:
            if dff.loc[i,B] == 0:
