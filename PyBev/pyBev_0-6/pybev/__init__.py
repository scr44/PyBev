...
def run_main_program():
    ...
    mcap = None
    metrics = None
    mcap, metrics = main_menu(mcap,metrics)
...
def main_menu(mcap,metrics):
    ...
    while True:        
        ...
        if type(user_input) is int:
            mcap,metrics = program_stages(
                                user_input,mcap,metrics)
...
def options_menu_gen():
 ...
 5. Default Flash Metrics Path: %s
 6. Default Archive Path: %s
 7. Default BASS Archive Path: %s
 8. Default Cloud Path: %s
...
""" % (
    ...
    get_setting('ARCHIVE','flash_path'),
...
def options_menu():    
    ...
    while i is not None:
        ...
        if i == 1:
            ...
            change_setting('METRICS','flash_path')
        elif i == 6:
...
def program_stages(stage,mcap,metrics):
    if stage == 1:
...
def stage_5(mcap,metrics):
    ...
    # Review items
    review_items_found = ts.non_qcc_items(mcap,metrics,alt_mcap,'Review')
    ...
    # Indexed items
    indexed_items_found = ts.non_qcc_items(mcap,metrics,alt_mcap,'Indexed')
...
def get_metrics(panel):
    if panel == 'Beval':
        ...
        return get_setting('METRICS','flash_path')
    else:
...
def create_settings():
    ...
    documents = r'C:\Users'+'\\'+getpass.getuser()+r'\Documents'
    config = configparser.ConfigParser()
    ...
    config.set('METRICS','bap_path', (documents+r'\Metrics\2019 %s Metrics.xlsx' % 'Beval'))
    config.set('METRICS','nonflash_path', (documents+r'\Metrics\2019 %s Metrics.xlsx' % 'Nonflash'))
    config.set('METRICS','flash_path', (documents+r'\Metrics\2019 Flash.xlsx'))
